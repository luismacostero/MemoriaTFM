...
para una factorización de Cholesky de sobre una matriz de dimensión $1024 \times 1024$ usando  precisión simple y
dividida en bloques de $64 \times 64$ elementos. La gráfica superior muestra el estado
de las colas durante la ejecución del problema, mientras que la gráfica
inferior muestra la frecuencia del cluster de núcleos \LITTLE en cada momento 
de la ejecución del problema. Como se puede apreciar, al inicio de la ejecución
(cuando el número de tareas críticas es elevado), la frecuencia se reduce de
...
\comentario{Quizás mover las figuras repetidas a un anexo}
\begin{figure}
  \centering
...
al número máximo hasta el momento. Los experimentos realizados contemplan
desde un tamaño del $50\%$ del tamaño máximo, decreciendo en intervalos de
un $10\%$ hasta finalizar en un tamaño del $10\%$ respecto al tamaño
máximo. Mencionar que este porcentaje no se encuentra relacionado con el
tiempo de ejecución, sino con el número de tareas listas en cada
momento. Como dato informativo, la tabla~\ref{tab:tam-colas-tiempo} muestra
para cada configuración del tamaño de la matriz y bloque (columnas), y para
cada experimento lanzado (filas), el porcentaje del tiempo de ejecución en
el que el cluster se encuentra desactivado.
...
Las figuras~\ref{fig:p4-p5} muestran el comportamiento de las políticas P4
y P5 para la plataforma \odroid, combinando para cada configuración de
tamaño de matriz y bloque los distintos tamaños de cola
experimentados. Adicionalmente, se incorpora las medidas realizadas sobre
el planificador \botlev para poder comparar los resultados.\\
A grandes rasgos, es posible extraer un conjunto de conclusiones generales
a ambas políticas:

\begin{itemize}
\item Variar el momento en el que el cluster se desactiva tiene un impacto
  muy significativo en el rendimiento final de la ejecución, tanto para
  desactivar el cluster de cores LITTLE (política P4) como para desactivar
  el cluster de cores big (política P5). Si se comparan los resultados de
  ambas plataformas, la pérdida de rendimiento es mayor en la plataforma
  \juno frente a la plataforma \odroid. La causa de este hecho es la
  configuración de su cluster: al únicamente poseer 2 cores big
  (Cortex-A57) frente a los 4 que posee la plataforma \odroid (Cortex-A15),
  si se desactiva el cluster de cores LITTLE las tareas se reparten entre
  un menor número de cores, explotando un menor grado de paralelismo.
  Esta diferencia no es tan elevada en la política P5, pues al desactivar
  el cluster de cores big, ambas plataformas tiene el cluster LITTLE
  formado por el mismo número de núcleos, por lo que el grado de
  paralelismo que se consigue es el mismo.
\item Al igual que en las políticas anteriores, el impacto en la potencia
  instantánea es mucho mayor sobre la política que desactiva los cores big,
  frente a la que desactiva el cluster LITTLE. Además, al igual que el
  rendimiento, el impacto que tiene el punto en el que se desactiva el
  cluster sobre la potencia instantánea es muy elevado.
\end{itemize}


Como se puede apreciar en la figura, la política P4 encargada de desactivar
el cluster LITTLE no consigue ninguna mejora energética para ningún
experimento realizado, sino que además en todos ellos empeora el
resultado. Sin embargo, la política P5 consigue una eficiencia energética
similar a la ejecución normal con \botlev, y adicionalmente consigue reducir
la potencia instantánea media de la ejecución. Como se puede apreciar, la
eficiencia energética conseguida es independiente del tamaño elegido para
aplicar la política, causado por la enorme reducción de la potencia
instantánea media al desactivar el cluster big. Desactivar el cluster en
una fase temprana de la ejecución proporciona una gran reducción de la
potencia y obtiene una eficiencia energética similar a no desactivar el
cluster durante toda la ejecución, pero a costa de reducir el rendimiento
de la aplicación.
...
\begin{figure}
  \centering
    \begin{subfigure}{0.75\textwidth}
      \centering
      \fbox{\includegraphics[width=0.9\textwidth]{Plots/sched_results/P4_juno.eps}}
      \caption{P4}
    \end{subfigure}
    ...
    \begin{subfigure}{0.75\textwidth}
      \centering
      \fbox{\includegraphics[width=0.9\textwidth]{Plots/sched_results/P5_juno.eps}}
      \caption{P5}
    \end{subfigure}  
...
\todo{Falta añadir P6, y revisar / comentar algo más de p4-p5}
