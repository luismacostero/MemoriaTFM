...
Cuando un código anotado es ejecutado sobre el planificador Nanox, este
comienza creando un conjunto de hilos denominados \wts, los cuales van a
ser los encargados de ejecutar las distintas tareas que están listas para
ejecución. Al primero de los hilos creados se le denomina \emph{master
  thread}, y es el encargado de, aparte de ejecutar tareas listas como
cualquier otro \wt, ejecutar el código secuencial del programa
original. Mientras que no existan tareas listas para ser ejecutadas, el
resto de \wts permanecerán inactivos a la espera de su creación. Notar que
al contrario que el modelo de OpenMP, Nanox no crea un nuevo hilo por cada
tarea a ejecutar, sino que crea un conjunto de hilos al inicio de la
aplicación, y son estos los encargados de ejecutar todas las tareas del
programa, finalizando su vida al acabar la ejecución del programa.
...
\item El \wt informa al \emph{runtime} de la finalización de la tarea. Al
  finalizar la tarea, se comprueban qué nuevas tareas están listas para ser
  ejecutadas ya que se han satisfecho todas sus dependencias. Cuando se
  determina que una tarea está lista para ejecución, se informa al módulo
  encargado del planificador para que la almacene y trate de acuerdo a la
...
\item El \wt solicita una nueva tarea para ejecutar al módulo
  ...
  el \wt no sea el idóneo para ejecutar ninguna de las tareas existentes,
  este pasa a estado inactivo hasta la creación de nuevas tareas, evitando
  así consumir recursos de manera innecesaria.
\end{enumerate}

Durante la ejecución de una tarea, un \wt puede encontrarse una anotación
para la creación de una nueva tarea. Si esto ocurre, el \wt informa al
\emph{runtime} para que cree la tarea correspondiente, y la inserte al DAG
del problema, esperando a que todas sus dependencias sean satisfechas y la
nueva tarea esté lista para su ejecución.
...
planificadores de tareas que acompañan a OmpSs, StarPU, MAGMA, Kaapi y {\tt libflame}~\cite{libflameref} son capaces
